...
def test_multiple_stocks_missing_symbols():
...
def test_multiple_stocks_empty_symbols():
    response = client.get("/stocks?symbols=,,,")
    ...
    assert data["success"] is False
    assert data["status"] == 404
    assert data["message"] == "No stock data found."
    assert data["path"] == "/stocks"